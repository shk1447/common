...
PastStock.prototype.selectRecommend = function(params) {
    var query = "SELECT * FROM (SELECT category as `id`, name, price,change_rate, volume_power,volume_percent,CONCAT(prev_total_state, ',', total_state) as total_state,CONCAT(prev_current_state, ',', current_state) as current_state,supstance,prev_supstance,unixtime FROM ( SELECT current.*, current.volume / past.volume * 100 as volume_power, current.volume/current.total_volume*100 as volume_percent, past.prev_total_state, past.prev_current_state, past.supstance as prev_supstance FROM ( SELECT category, column_get(rawdata,'종목명' as char) as name, column_get(rawdata, '거래량' as double) as volume,  column_get(rawdata, '저항갯수' as double) as regist_count, column_get(rawdata, '지지갯수' as double) as support_count,column_get(rawdata, '전체상태' as char) as prev_total_state, column_get(rawdata, '현재상태' as char) as prev_current_state,column_get(rawdata, '지지가격대' as char) as supstance, unixtime FROM past_stock WHERE column_get(rawdata,'현재상태' as char) = '하락' AND unixtime >= '"+params.prev_id+"' AND unixtime <= '"+params.id+"') as past, ( SELECT category, column_get(rawdata, '지지가격대' as char) as supstance, column_get(rawdata, '종목명' as char) as name,  column_get(rawdata, '종가' as double) as price, column_get(rawdata, '시가' as double) as start_price, column_get(rawdata, '전일비율' as double) as change_rate,  column_get(rawdata, '거래량' as double) as volume,  column_get(rawdata, '전체상태' as char) as total_state,  column_get(rawdata, '현재상태' as char) as current_state, column_get(rawdata, '저항갯수' as double) as regist_count, column_get(rawdata, '지지갯수' as double) as support_count, column_get(rawdata, '상장주식수' as double) as total_volume, unixtime FROM past_stock WHERE column_get(rawdata,'현재상태' as char) = '상승' AND unixtime >= '"+params.id+"' AND unixtime <= '"+moment(params.id).add(1, 'day').format("YYYY-MM-DD")+"' AND column_get(rawdata, '거래량' as double) / column_get(rawdata, '상장주식수' as double) * 100 > 1) as current WHERE past.category = current.category AND current.regist_count < current.support_count AND past.regist_count >= current.regist_count AND past.support_count <= current.support_count AND current.start_price < current.price) as test) as result ORDER BY volume_percent";
